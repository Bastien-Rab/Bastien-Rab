...
|[Projet data pipeline retail](https://github.com/Bastien-Rab/data-pipeline-retail) | (EN COURS) Construire un pipeline de données de type ELT moderne, simulant l’environnement d’une entreprise retail, orchestré avec Airflow, transformé avec DBT, et stocké dans un entrepôt (Snowflake ou PostgreSQL/SQLite local). | Python, PostgreSQL, DBT, Airflow, Github|
...
| [Retail Data Pipeline Project](https://github.com/Bastien-Rab/data-pipeline-retail) | (IN PROGRESS) Building a modern ELT-style data pipeline, simulating the data environment of a retail company, orchestrated with Airflow, transformed with DBT, and stored in a data warehouse (Snowflake or local PostgreSQL/SQLite). | Python, PostgreSQL, DBT, Airflow, GitHub |
